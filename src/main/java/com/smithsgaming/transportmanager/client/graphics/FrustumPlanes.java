...
class FrustumPlanes {
    ...
    public boolean planeAABBEquation(Vector3f aabbMin, Vector3f aabbMax) {
        ...
        Vector3f center = new Vector3f(aabbMin).add(aabbMax).mul(.5f);
        Vector3f extent = new Vector3f(aabbMax).sub(aabbMin).mul(.5f);
        float maxExtent = (float) Math.sqrt(extent.dot(extent));
    ...
    public void normalize() {
        ...
        invMag = (float) (1f / Math.sqrt(npX * npX + npY * npY + npZ * npZ));
        npW *= invMag; // We still normalize it though
        npX *= invMag;
        npY *= invMag;
        npZ *= invMag;
        invMag = (float) (1f / Math.sqrt(fpX * fpX + fpY * fpY + fpZ * fpZ));
        fpW *= invMag;
        fpX *= invMag;
        fpY *= invMag;
        fpZ *= invMag;
        invMag = (float) (1f / Math.sqrt(lpX * lpX + lpY * lpY + lpZ * lpZ));
        lpW *= invMag;
        lpX *= invMag;
        lpY *= invMag;
        lpZ *= invMag;
        invMag = (float) (1f / Math.sqrt(rpX * rpX + rpY * rpY + rpZ * rpZ));
        rpW *= invMag;
        rpX *= invMag;
        rpY *= invMag;
        rpZ *= invMag;
        invMag = (float) (1f / Math.sqrt(bpX * bpX + bpY * bpY + bpZ * bpZ));
        bpW *= invMag;
        bpX *= invMag;
        bpY *= invMag;
        bpZ *= invMag;
        invMag = (float) (1f / Math.sqrt(tpX * tpX + tpY * tpY + tpZ * tpZ));
        tpW *= invMag;
        tpX *= invMag;
        tpY *= invMag;
        tpZ *= invMag;
